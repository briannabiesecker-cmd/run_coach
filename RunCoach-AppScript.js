...
// ──────────────────────────────────────────────────
// SAMPLE PLAN POPULATOR — Higdon Novice 1 marathon
// ──────────────────────────────────────────────────
//
// Mirror of the frontend buildHigdonNovice1Marathon() function. Builds
// a complete 18-week marathon plan in the same JSON shape that Gemini
// produces, so the rest of the app treats it identically. Used by
// populateSampleSheet() below to write to a specific sheet without
// going through the cloud sync flow.

function buildHigdonNovice1Plan(raceDateISO) {
  // M rest, T easy, W easy, Th easy, F rest, Sa long, Su cross
  var grid = [
    [ 1,  3, 3, 3, 6 ],
    [ 2,  3, 3, 3, 7 ],
    [ 3,  3, 4, 3, 5 ],
...
// ──────────────────────────────────────────────────
// populateSampleSheet — runnable from the Apps Script editor.
// Writes the Higdon Novice 1 plan to a specific sheet in two formats:
//   - "data" tab: JSON blob (used by the app's cloud sync — same shape
//                  saveUserData would produce)
//   - "Plan" tab: human-readable rows (Week | Phase | Day | Type |
//                  Miles | Pace | EstMin | Note)
//
// Default sheet ID is the user-shared one. To populate a different
// sheet, edit DEFAULT_SHEET_ID below or set the SAMPLE_SHEET_ID
// script property.
//
// Usage from the Apps Script editor:
//   1. Function dropdown → populateSampleSheet → Run
//   2. Check the execution log for the success message + sheet URL
function populateSampleSheet() {
  ...
  // Compute a default race date: 18 weeks from this Monday
  var d = new Date();
  var dayOfWeek = (d.getDay() + 6) % 7; // Mon=0..Sun=6
  ...
  if (!dataSheet) {
    ...
    if (dataSheet.getRange(1, 1).getValue() !== 'payload') {
      dataSheet.getRange(1, 1, 1, 2).setValues([['payload', 'updatedAt']]);
      dataSheet.setFrozenRows(1);
    }
  }
  // Wrap the plan in the same payload shape that saveUserData writes
  // so the app can read it back via loadUserData with no migration.
  var payload = {
    plan: plan,
  ...
  if (planSheet) {
    // Wipe and rewrite — easier than reconciling diffs
    ss.deleteSheet(planSheet);
  }
  ...
  for (var w = 0; w < plan.weeks.length; w++) {
    ...
    for (var di = 0; di < week.days.length; di++) {
      ...
      // Only show focus on first day of week to reduce noise
  ...
  // Light formatting: bold header, auto-resize columns, color phases
  ...
  // Color rows by phase (subtle background)
  var phaseColors = { 'Base': '#ecfdf5', 'Build': '#eff6ff', 'Peak': '#fff7ed', 'Taper': '#faf5ff' };
  for (var r = 0; r < rows.length; r++) {
    var color = phaseColors[rows[r][1]];
    if (color) planSheet.getRange(r + 2, 1, 1, headers.length).setBackground(color);
  }

  // ── Tab 3: "Plan summary" — phases, paces, holistic guidance ──

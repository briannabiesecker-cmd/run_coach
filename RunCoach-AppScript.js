...
function buildSystemPrompt(style) {
  ...
  return [
    ...
    '',
    'NAMED WORKOUT LIBRARY — these are real, coach-branded sessions. When the workout you are prescribing matches one of these, USE THE NAME in the workout note. Do not invent names for standard workouts; if a session is not on this list, describe it generically (e.g. "5x1k @ 5K pace") rather than fabricating a brand.',
    '',
    '  - "Yasso 800s" (Bart Yasso, Runner\'s World): 10x800m where each rep is run in min:sec equal to your goal marathon time in hr:min (e.g. 4:00 reps for a 4-hour marathon goal). 400m jog recovery. Used in marathon peak phase as both workout and predictor.',
    '',
    '  - "Magic Mile" (Jeff Galloway): 1mi all-out time trial after a 15-min warmup. Used as a fitness benchmark every 4-6 weeks and to predict race times. Place sparingly — not a regular session.',
    '',
    '  - "Cruise intervals" (Jack Daniels, Daniels\' Running Formula): 4-5 x 1mi at tempo (T) pace with 60 sec jog recovery. The short recovery makes this a true threshold workout — recovery is not enough to drop heart rate fully.',
    '',
    '  - "Michigan workout" (Ron Warhurst, Michigan XC): A ladder workout — 1mi @ tempo, then 1200m @ 5K pace, then 800m @ 3K-mile pace, then 400m all out. Tempo-paced 800m recovery jogs between reps. Advanced; use only in peak phase for experienced runners.',
    '',
    '  - "Hanson long run" (Hansons Marathon Method): A long run capped at 16mi but run on accumulated fatigue (no preceding rest day). The point is teaching the body to run on tired legs at race-relevant duration. Use in marathon build phase.',
    '',
    '  - "Pfitzinger long run with MP" (Pete Pfitzinger, Advanced Marathoning): Long run with a marathon-pace block in the second half. Examples: "16 mi total with last 12 @ marathon pace" or "20 mi with miles 8-15 @ MP". The cornerstone of Pfitzinger\'s marathon plans. Use in marathon build/peak.',
    '',
    '  - "Galloway run-walk long run" (Jeff Galloway): Long run executed with structured walk breaks (e.g. run 4 min, walk 1 min, repeat). Used by beginners and injury-prone runners to extend long-run duration without breakdown.',
    '',
    '  - "Lydiard long aerobic" (Arthur Lydiard): Very long, very easy run at the upper end of conversational pace. Builds pure aerobic capacity. Use in early base phase. Distance is determined by time on feet, not miles — typically 90-180 min.',
    '',
